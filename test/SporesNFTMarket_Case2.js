...
//      + Phase 2: Release SporesNFTmarketv3 supports Buy/Sell NFT1155
...
- Case 2:
    ...
    + Phase 1: SporesNFTMarket supports Buy/Sell NFT721
    + Phase 2: Release SporesNFTMarketv3 supports both Buy/Sell NFT721-NFT1155
        SporesNFTMarket is deprecated
***********************************************************************************************************************/

describe('SporesNFTMarket Contract Testing - Buy NFT721/NFT1155 with ERC20/Native Coin - Case 2', () => {
    let provider;
    let deployer, seller, buyer, verifier, feeCollector;
    let token721, token1155, minter, market, marketv3, registry;
    ...
    before(async() => {
        ...
        const SporesNFTMinter = await ethers.getContractFactory('SporesNFTMinterV2', deployer);
    ...
    ***************************************************************************************************/
    ...
    it('Should succeed to deploy SporesNFTMarketv3', async() => {
        //  Deploy and initialize SporesNFTMarketv3 contract
        //  SporesNFTMarketv3 contract is written following non-upgradeability feature
        //  Hence, constructor is defined and being called when deploying SporesNFTmarketv3 contract
        const SporesNFTMarketv3 = await ethers.getContractFactory('SporesNFTMarketv3', deployer);
        marketv3 = await SporesNFTMarketv3.deploy(registry.address);

        //  Update new Market contract into SporesRegistry
        await registry.updateMarket(marketv3.address);
    });
    ...
    it('Should succeed purchase NFT721 with native coin - After deploying SporesNFTMarketv3', async() => {
        //  Prepare input data, and send a minting request
        ...
        //  Seller setApproveForAll to allow SporesNFTMarket transfer NFT721 item
        await token721.connect(seller).setApprovalForAll(marketv3.address, true);
        ...
        //  Buyer makes a purchase
        const tx = await marketv3.connect(buyer).buyNFT721NativeCoin(
            info, signature2,
            {
                value: price
            }
    ...
    it('Should succeed purchase NFT721 with ERC-20 - After deploying SporesNFTmarketv3', async() => {
        //  Prepare input data, and send a minting request
        ...
        //  Buyer setApproveForAll to allow SporesNFTMarket transfer ERC20
        await erc201.connect(buyer).approve(marketv3.address, 1000000000000000);
        ...
        //  Buyer makes a purchase
        const tx = await marketv3.connect(buyer).buyNFT721ERC20(
            info, signature2
        );
        const receipt = await tx.wait();
        let event = receipt.events.find(e => { return e.event == 'SporesNFTMarketTransaction' });
    ...
    it('Should succeed purchase NFT1155 with native coin', async() => {
        ...
        await token1155.connect(seller).setApprovalForAll(marketv3.address, true);
        ...
        //  Buyer makes a purchase
        const tx = await marketv3.connect(buyer).buyNFT1155NativeCoin(
            info, signature2,
            {
    ...
    it('Should revert when purchase NFT1155 with invalid amount - Invalid msg.value', async() => {
        ...
        //  Buyer makes a purchase with invalid msg.value
        await expect(
            marketv3.connect(buyer).buyNFT1155NativeCoin(
                info, signature2, 
                {
    ...
    it('Should revert when token1155 - param and signed by Verifier - not matched', async() => {
        ...
        //  Seller setApproveForAll to allow SporesNFTMarket transfer NFT1155 item
        await invalidToken1155.connect(seller).setApprovalForAll(marketv3.address, true);
    ...
    it('Should revert when Seller has not yet setApprovalForAll', async() => {
        ...
        await token1155.connect(seller).setApprovalForAll(marketv3.address, false);
    ...
    it('Should succeed purchase NFT1155 with ERC20', async() => {
        ...
        //  Buyer setApproveForAll to allow SporesNFTMarket transfer ERC20
        await erc203.connect(buyer).approve(marketv3.address, 1000000000000000);
        ...
        //  Buyer makes a purchase
        const tx = await marketv3.connect(buyer).buyNFT1155ERC20(info, signature2);
        const receipt = await tx.wait();
        let event = receipt.events.find(e => { return e.event == 'SporesNFTMarketTransaction' });
    ...
    it('Should revert when purchase NFT1155 with invalid payment token', async() => {
        ...
        //  Buyer setApproveForAll to allow SporesNFTMarket transfer ERC20
        await erc204.connect(buyer).approve(marketv3.address, 1000000000000000);
        ...
        //  Buyer makes a purchase with invalid msg.value
        await expect(
            marketv3.connect(buyer).buyNFT1155ERC20(
                info, signature2
            )    
        ).to.be.revertedWith('SporesNFTMarket: Invalid payment');
    ...
    //  TODO
    it(`Skipped - Should revert when Seller - param and signed by Verifier - not matched`, async() => {});
