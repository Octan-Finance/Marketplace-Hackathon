...
//      + Phase 1: SporesNFTMarket supports Buy/Sell NFT721
//      + Phase 2: 
//          * SporesRegistry is upgraded
//          * Release SporesNFTMarketUpgradeTest

/**********************************************************************************************************************
- Case 1: This case is deprecated. SporesRegistry saves SporesNFTMarket contract address
    When a new market contract is deployed, Admin calls updateMarket() to update a new address
    Thus, old market contract is likely failed to link to SporesRegistry
    + SporesRegistry is fixed (non-upgrade)
    + Phase 1: SporesNFTMarket supports Buy/Sell NFT721
    + Phase 2: Release SporesNFTMarketv2 supports Buy/Sell NFT1155
        Both contract is running concurrently
...
describe.skip('SporesNFTMarket Contract Testing - Buy NFT721/NFT1155 with ERC20/Native Coin - Case 1', () => {
    let provider;
    let deployer, seller, buyer, verifier, feeCollector;
    let token721, token1155, minter, market, marketv2, registry;
    ...
    before(async() => {
        ...
        [deployer, seller, buyer, verifier, feeCollector, anotherSeller] = await ethers.getSigners();
        ...
        //  Add Market and Minter into SporesRegistry
    ...
    ***************************************************************************************************/
    ...
    it('Should revert when purchase NFT721 with invalid Price - Invalid msg.value', async() => {
        ...
        await expect(
            market.connect(buyer).buyNFT721NativeCoin(
                info, signature2, { value: invalidPrice }
            )    
    ...
    it('Should revert when purchase NFT721 with invalid Price', async() => {
        ...
        ).to.be.revertedWith('SporesNFTMarket: Invalid Verifier signature');
    ...
    it('Should revert when purchase NFT721 with invalid tokenId - Seller Not Owned', async() => {
        ...
        const sellId = 18002080;
        const signature2 = await verifyPurchaseSignature(
            verifier, anotherSeller.address, seller.address, token721.address, tokenId,
            ethers.constants.AddressZero, feeRate, price, SINGLE_UNIT, sellId, NATIVE_COIN_NFT_721
        );
        ...
        //  Buyer makes a purchase with invalid tokenId - Seller now owned
        await expect(
            market.connect(buyer).buyNFT721NativeCoin(
                info, signature2, { value: price }
            )    
    ...
    it('Should revert when token721 - param and signed by Verifier - not matched', async() => {
        ...
        await registry.registerNFTContract(invalidToken721.address, opcode);
    ...
    it('Should succeed purchase NFT721 with ERC-20', async() => {
        ...
        //  Buyer makes a purchase
        const tx = await market.connect(buyer).buyNFT721ERC20(
            info, signature2
        );
        const receipt = await tx.wait();
        let event = receipt.events.find(e => { return e.event == 'SporesNFTMarketTransaction' });
    ...
    ***************************************************************************************************/
    ...
    it('Should succeed to deploy SporesNFTMarketv2', async() => {
        //  Deploy and initialize SporesNFTMarketv2 contract
        //  SporesNFTMarketv2 contract is written following non-upgradeability feature
        //  Hence, constructor is defined and being called when deploying SporesNFTMarketv2 contract
        const SporesNFTMarketv2 = await ethers.getContractFactory('SporesNFTMarketv2', deployer);
        marketv2 = await SporesNFTMarketv2.deploy(registry.address);

        expect(await marketv2.VERSION()).deep.equal(ethers.utils.solidityKeccak256(['string'],['MARKET_1155_v1']));
        expect(await marketv2.status()).deep.equal(STATUS.ACTIVE);

        //  Update new Market into SporesRegistry
        await registry.updateMarket(marketv2.address);
    });

    it('Should succeed purchase NFT721 with native coin - After deploying SporesNFTMarketv2', async() => {
        //  Prepare input data, and send a minting request
        const tokenId = 7210020;
        const uri = 'https://test.metadata/7210020';
        const signature1 = await verifySignature(verifier, seller.address, tokenId, uri, ERC721);
        const mintTx = await minter.connect(seller).mintSporesERC721(tokenId, uri, signature1);
        await mintTx.wait();
        ...
        //  Check balance of Seller, Buyer before purchase
        const balanceOfSellerBefore = await provider.getBalance(seller.address);
        expect(await token721.balanceOf(seller.address)).deep.equal(5);
        expect(await token721.balanceOf(buyer.address)).deep.equal(2);
        expect(await token721.ownerOf(tokenId)).deep.equal(seller.address);
        ...
        const balanceOfSellerAfter = await provider.getBalance(seller.address);
        expect(await token721.balanceOf(seller.address)).deep.equal(4);
        expect(await token721.balanceOf(buyer.address)).deep.equal(3);
        expect(await token721.ownerOf(tokenId)).deep.equal(buyer.address);
        expect(balanceOfSellerBefore.add(BigNumber.from(payToSeller))).deep.equal(balanceOfSellerAfter);
    });

    it('Should succeed purchase NFT721 with ERC-20 - After deploying SporesNFTMarketv2', async() => {
        //  Prepare input data, and send a minting request
        const tokenId = 7210030;
        const uri = 'https://test.metadata/7210030';
        const signature1 = await verifySignature(verifier, seller.address, tokenId, uri, ERC721);
        const mintTx = await minter.connect(seller).mintSporesERC721(tokenId, uri, signature1);
        await mintTx.wait();
        ...
        const balanceOfSellerAfter = await erc201.balanceOf(seller.address);
        expect(await token721.balanceOf(seller.address)).deep.equal(4);
        expect(await token721.balanceOf(buyer.address)).deep.equal(4);
        expect(await token721.ownerOf(tokenId)).deep.equal(buyer.address);
        expect(balanceOfSellerBefore.add(BigNumber.from(payToSeller))).deep.equal(balanceOfSellerAfter);
        expect(balanceOfBuyerBefore.sub(BigNumber.from(price))).deep.equal(balanceOfBuyerAfter);
    });
    ...
    it('Should succeed purchase NFT1155 with native coin', async() => {
        ...
        const amount = 1101;
        const signature1 = await verifySignature(verifier, seller.address, tokenId, uri, ERC1155);
        const mintTx = await minter.connect(seller).mintSporesERC1155(tokenId, amount, uri, signature1);
        await mintTx.wait();

        //  Seller setApproveForAll to allow SporesNFTMarket transfer NFT1155 item
        await token1155.connect(seller).setApprovalForAll(marketv2.address, true);
        ...
        //  Buyer makes a purchase
        const tx = await marketv2.connect(buyer).buyNFT1155NativeCoin(
            info, signature2, { value: BigNumber.from(price).mul(purchaseAmt) }
        );
        const receipt = await tx.wait();
        let event = receipt.events.find(e => { return e.event == 'SporesNFTMarketTransaction' });
    ...
    it('Should revert when purchase NFT1155 with invalid amount - Invalid msg.value', async() => {
        ...
        //  Buyer makes a purchase with invalid msg.value
        await expect(
            marketv2.connect(buyer).buyNFT1155NativeCoin(
                info, signature2, { value: BigNumber.from(price).mul(purchaseAmt - 1) }
            )    
        ).to.be.revertedWith('SporesNFTMarket: Insufficient payment');
    ...
    it('Should revert when purchase NFT1155 with invalid amount - Invalid Price', async() => {
        ...
        await expect(
            marketv2.connect(buyer).buyNFT1155NativeCoin(
                info, signature2, { value: BigNumber.from(invalidPrice).mul(purchaseAmt) }
            )    
        ).to.be.revertedWith('SporesNFTMarket: Invalid Verifier signature');
    ...
    it('Should revert when purchase amount - param and signed by Verifier - not matched', async() => {
        ...
        await expect(
            marketv2.connect(buyer).buyNFT1155NativeCoin(
                info, signature2, { value: BigNumber.from(price).mul(invalidPurchaseAmt) }
            )    
        ).to.be.revertedWith('SporesNFTMarket: Invalid Verifier signature');
    ...
    it('Should revert when tokenId - param and signed by Verifier - not matched', async() => {
        ...
        const signature1 = await verifySignature(verifier, seller.address, tokenId2, uri, ERC1155);
        const mintTx = await minter.connect(seller).mintSporesERC1155(tokenId2, amount, uri, signature1);
        await mintTx.wait();
        ...
        const purchaseAmt = 10;
        const signature2 = await verifyPurchaseSignature(
            verifier, seller.address, seller.address, token1155.address, tokenId1,
            ethers.constants.AddressZero, feeRate, price, purchaseAmt, sellId, NATIVE_COIN_NFT_1155
        );
    ...
    it('Should revert when token1155 - param and signed by Verifier - not matched', async() => {
        ...
        await registry.registerNFTContract(invalidToken1155.address, opcode);
        ...
        //  Seller setApproveForAll to allow SporesNFTMarket transfer NFT1155 item
        await invalidToken1155.connect(seller).setApprovalForAll(marketv2.address, true);
        ...
        const purchaseAmt = 10;
        const signature2 = await verifyPurchaseSignature(
            verifier, seller.address, seller.address, invalidToken1155.address, tokenId,
            ethers.constants.AddressZero, feeRate, price, purchaseAmt, sellId, NATIVE_COIN_NFT_1155
        );
    ...
    it('Should revert when Seller has not yet setApprovalForAll', async() => {
        ...
        await token1155.connect(seller).setApprovalForAll(marketv2.address, false);
    ...
    it('Should succeed purchase NFT1155 with ERC20', async() => {
        ...
        //  Buyer makes a purchase
        const tx = await marketv2.connect(buyer).buyNFT1155ERC20(info, signature2);
        const receipt = await tx.wait();
        let event = receipt.events.find(e => { return e.event == 'SporesNFTMarketTransaction' });
    ...
    it('Should revert when purchase NFT1155 with invalid payment token', async() => {
        ...
        //  Buyer setApproveForAll to allow SporesNFTMarket transfer ERC20
        await erc204.connect(buyer).approve(marketv2.address, 1000000000000000);
        ...
        //  Buyer makes a purchase with invalid payment token
        await expect(
            marketv2.connect(buyer).buyNFT1155ERC20(
                info, signature2
            )    
        ).to.be.revertedWith('SporesNFTMarket: Invalid payment');
    ...
    //  TODO:
    it(`TODO - Should revert when Seller - param and signed by Verifier - not matched
        Likely Not Happend For Spores NFT1155 - Should test for others`, async() => {});
